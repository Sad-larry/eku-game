...
## 参数：room - 目标房间实例，cell - 单元格数据（包含事件类型）
...
## 功能：生成普通战斗事件
## 参数：room - 房间实例，cell - 单元格数据（用于难度调整和随机种子）
...
## 参数：room - 房间实例，cell - 单元格数据（用于随机种子），count - 生成敌人数量
...
## 功能：将预制体实例化并添加到指定容器中
## 参数：container - 目标容器节点，scene - 待实例化的预制体
